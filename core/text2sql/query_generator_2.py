...
class Text2SQL(QdrantVectorStore, SQLConnector,AddTableContext,Schema2Chunks):
    ...
    def __init__(self,model_name,api_key,db_type,host,port,username,password,database,db_location=None, qdrant_path=None,dense_model="sentence-transformers/all-MiniLM-L6-v2", sparse_model="prithivida/Splade_PP_en_v1", hybrid=True,override_existing_index=False,max_attempts=5,add_additional_context=True) -> None:
        
        self.api_key = os.getenv("MISTRAL_API_KEY") or api_key
        ...
        if not qdrant_path:
            ...
            QdrantVectorStore.__init__(self, db_location=qdrant_path,collection_name=self.collection_name, dense_model=dense_model, sparse_model=sparse_model, hybird=hybrid)
        
        # Initialize SQLConnector
        SQLConnector.__init__(self,db_type,host,port,username,password,database)

        AddTableContext.__init__(self,model_name,api_key=self.api_key)
        ...
        self.instructor_client = instructor.from_mistral(MistralClient(api_key=self.api_key))
    ...
    def __connect_to_db(self):
        ...
        if self.schema_description is None:
            raise ValueError("Failed to connect to the database or fetch schema details. Please check your database credentials and ensure the database is running.")
