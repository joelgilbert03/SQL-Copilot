...
class MistralChatModel:
    ...
    def __init__(self,model_name="mistral-large-latest",api_key=None,temperature=0.5,max_tokens=3000,max_retries=3,verbose=False) -> None:
        self.model_name = model_name
    ...
    async def aget_output(self, messages):
        ...
        for attempt in range(self.max_retries):
            if self.verbose:
                print_colored("Hitting The Mistral API....", "blue")
            ...
            if self.verbose:
                print_colored(f"Total Token Usage {response.usage.total_tokens}", "blue")
...
# MISTRAL CHANGE START
class MistralVisionModel:
    def __init__(self, model='pixtral-large-latest', api_key=None, max_tokens=3000):
        self.model = model
        self.api_key = api_key or os.getenv("MISTRAL_API_KEY")
        self.max_tokens = max_tokens
        self.async_client = MistralAsyncClient(api_key=self.api_key)
    ...
    async def get_output(self, question, image_path=None, base64_image=None):
        """Analyzes the content of an image using Mistral AI."""
